...
class Solution {
    ...
    private int binarySearch(int[] arr, int x) {
        ...
        return res; // 如果不用res 可以直接return right;
    } 
...
### 写法三 (三种写法中最优, 推荐)
写法一&二比较好写，但是res.add(0, num) 非常耗费时间，所以同样的原理，这种写法最优：
```java
class Solution {
    public List<Integer> findClosestElements(int[] arr, int k, int x) {
        int left = binarySearch(arr, x);
        ...
        for (int i = left + 1; i < right; i++) { // 为什么是left+1 & right, 自己走一遍
            ans.add(arr[i]);
        }
        return ans;
    }
