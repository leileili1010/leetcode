...
class Solution {
    public int jobScheduling(int[] startTime, int[] endTime, int[] profit) {
        ...
        for (int i = 1; i <= n; i++) {
            ...
            int compatible = 0;
            for (int j = i - 2; j >= 0; j--) { // 因为cur job是jobs[i-1],cur job前面的jobs 是 jobs[i-1] ~ jobs[0]; for loop starts from the job immediately before the current one (i - 2) and moves backward.
                if (jobs[j][1] <= start) {
