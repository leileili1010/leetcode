...
- time: 
    - set: O(1)
    - get: O(logM), 某个 key 有 M 个 timestamps
- space: O(N)
```java
class TimeMap {
    ...
    public void set(String key, String value, int timestamp) { // time: O(1)
        map.computeIfAbsent(key, k -> new ArrayList<>()).add(new Pair(timestamp, value));
    }
    
    public String get(String key, int timestamp) { // time: O(logM), 某个 key 有 M 个 timestamps
        if (!map.containsKey(key)) return "";
        List<Pair> list = map.get(key);
        int left = 0, right = list.size()-1;
        String res = "";

        // 求最大的小于等于 timestamp 的值
            // t.timestamp <= timestamp，说明当前mid是一个合法答案，但还要向右边看看有没有更大的合法答案
            // res 始终存的是“当前为止最好的答案”，二分会继续往右找是否存在更接近 timestamp 的 timestamp
        while (left <= right) {
