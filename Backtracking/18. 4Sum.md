...
当 k = 2时， time complexity almost = O(N) after pruning so total time is O(N^3)
...
```
### Optimization final - 剪枝 + binary search (最推荐 for best performance)
- 这种解法不仅仅适用于4Sum, 也是用于kSum
