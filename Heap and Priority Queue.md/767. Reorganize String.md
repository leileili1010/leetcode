...
类似题目：LC621 task scheduler

## Approach 1 - 奇偶填法（最优解法）

- time: O(N)
- space: O(1)

idx >= n 的判断，是为了“动态决定什么时候该从偶数位切换到奇数位”，而不是简单的阶段切换
- e.g s = "aabbc", n = 5, 
- a_ a _ _ => a _ a _ b => a b a c b
...
class Solution {
    public String reorganizeString(String s) {
        ...
        for (int i = 0; i < 26; i++) {
            while (freq[i]-- > 0) {
                if (idx >= n) idx = 1; // 切换到奇数位，这里很tricky
                res[idx] = (char) (i + 'a');
                idx += 2;
            }
        }
...
class Solution {
    public String reorganizeString(String s) {
        ...
        return sb.length() == s.length() ? sb.toString() : ""; // 重点是这里
    }
}
```
